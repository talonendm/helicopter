...
// tekoalykopterille helppokoodata.
// hieman kaasua, jos yläpuolella toinen.. 
// ja käännä hieman kopteria.. ampuu kun melkein hollilla silloin tällöin. Vaikeustasosta riippuen.

float radius = 50.0;
int X, Y;
int nX, nY;
int painovoima = 2;//0.01;//1;   // jotain vikaa ampumisessa.. cos ja sinit sekaisin..
...
int maxkaasu = 4;
int kaasuntasoitus = 0.015; // 0== tällöin alkuperäinen


int tekoalyapu;
int tekoalyapu2;

int kaasuviivakerroin = 100/maxkaasu;


//int delay = 4;
int delay = 10;   // oltava 1...

int koko = 120; // kopterinkoko
int oikeakoko = koko/2.3; // SVG-piirturi on paska ja tyhjaa tulee
int potkurikoko = koko/2;
int potkurikerroin = 13;
int oikeasade = oikeakoko/2;


// kokeellinen SVG-graffihieman huonohko..
int tykkixsijainti = (oikeakoko-5)/2;
int tykkiysijainti = oikeakoko-5;
...
int kuvaleveys = screen.width-17;
int kuvakorkeus = screen.height*0.88-120;
...
sun = loadShape("sunv2.svg");
sun2 = loadShape("sun2v2.svg");
sun3 = loadShape("sun3v2.svg");

smoke = loadShape("lowenergysmoke.svg");
smoke2 = loadShape("lowenergysmoke2.svg");
...
size(kuvaleveys, kuvakorkeus);
 ...
 // size( screen.width, screen.height );
  ...
  aurinkox = width-aurinkokoko-aurinkoy;



  aurinkokx = aurinkox + aurinkokoko/2;
  aurinkoky = aurinkoy + aurinkokoko/2;
  aurinkoactive = 0;
...
//numbers[2] = 30; 
  ...
  myFont = createFont("FFScala", floor(kuvaleveys/60));
  textFont(myFont);
  //text("!@#$%", 10, 50);
...
if (aurinkoactive == 0) {
	...
	shape(sun3,aurinkox,aurinkoy,aurinkokoko,aurinkokoko);
}
...
//xv = xv - this.kaasu*cos(asento)*suunta;
  ...
  //  yv = yv*ilmanvastus;
	...
	tykkix = xpos + tykkixsijainti*suunta*(cos((asento)) - sin(asento));
	tykkiy = ypos + tykkiysijainti*(sin(asento) + cos(asento));
	...
	if (player == 2){
			...
			keskiosax = h1.xpos + oikeakoko/10*h1.suunta*(cos((h1.asento)) - sin(h1.asento));
			keskiosay = h1.ypos + oikeasade*(sin(h1.asento) + cos(h1.asento));


		} else {
			keskiosax = h2.xpos + oikeakoko/10*h2.suunta*(cos((h2.asento)) - sin(h2.asento));
			keskiosay = h2.ypos + oikeasade*(sin(h2.asento) + cos(h2.asento));
		}
...
void update() { 
	...
	// 6.6.2012 - rotate ei toimi kuin maassa=!?? toimii
	// *************************************
	if (player == 2) {
	...
	if (aurinkoactive == 2) {
		...
		tekoalyapu2 = -(h1.ypos - h2.ypos)/4000;
		
		if (yv>0) {
			tekoalyapu = tekoalyapu2 + yv/5;
		} else {
			tekoalyapu = tekoalyapu2;
		}

		if (tekoalyapu>kaasuaskel/5) {
			tekoalyapu = kaasuaskel/5;
		}
		if (tekoalyapu<-kaasuaskel/5) {
			tekoalyapu = -kaasuaskel/5;
		}
		kaasu = kaasu + tekoalyapu;
		...
		// LISÄKS SUUNTIEN KAA JOTAINH HÄIKKÄÄ... 7.6.2012

		// pitäis kattoo tykin suunnan eikä vaan pelkkä korkeus ero...
		...
		if (abs(asento-HALF_PI)<PI/6){

		
			tekoalyapu2 = tykkiy-keskiosay;

		//if (tekoalyapu2<0) {
		if (tykkiy-keskiosay<0) {
			tekoalyapu2 = kaantyykerralla*suunta/50;
			...
			asento -=tekoalyapu2;
	   		s21.rotate(-tekoalyapu2);  
			s22.rotate(tekoalyapu2);  	
		...
		if ((suunta==-1) && (h1.xpos - h2.xpos)< -100){
			suunta = 1;
		} if ((suunta==1) && (h1.xpos - h2.xpos)> 100){
			suunta = -1;
		}
		...
		if (xpos<20){
			...
			asento +=kaantyykerralla*suunta/2;
	        	s21.rotate(kaantyykerralla*suunta/2);  
			s22.rotate(-kaantyykerralla*suunta/2);  

		}
		if (xpos>kuvaleveys-20){
			suunta = 1;
		...
		//println(suunta);
			...
			// tekoalyampuu sillo tällö jos riittävälähel

	// ampuuko vai ei
	if ((random(1)<0.2) && (energia>0)){
...
//println(keskiosax);
    ...
    kaasu = kaasu + (painovoima-kaasu)*kaasuntasoitus;
  ...
  if (xpos > kuvaleveys) { 
   ...
   if (this.player == 2){
    ...
    if (energia<50)  {
        ...
        if ((energia > 0) || (yv>-0.0001)) { 	


		if (energia<30) {
			shape(smoke2, xpos+ random(3)-xv*2+oikeakoko/2*suunta, ypos+ random(6)-yv*2-oikeakoko/2, random(8)+(70-energia), random(5)+(100-energia));
		}
		//if (random(1)<0.5) {
   		
		shape(smoke, xpos + random(3)-xv*2+oikeakoko/2*suunta, ypos+ random(6)-yv*2-oikeakoko/2, random(8)+(90-energia), random(5)+(90-energia));
		//	} else {
...
color c1 = color((2-player)*222, (player-1)*222, 0);
 ...
 line(apu, maakorkeus+30,apu+panoksia*(player*2-3),maakorkeus+30);
    ...
    line(apu, maakorkeus+10,apu+kaasu*kaasuviivakerroin*(player*2-3),maakorkeus+10);
...
// tanne vois pistaa tormayksen.. tosin pitäis varmaan pirtää uudet kopterit. Rasittaa tuo tyhjä alue.. pitäis ottaa huomioon törmäyksessäkin
// **************************************
// törmäys
// **************************************


//ellipse((h2.xpos + oikeakoko/10*h2.suunta*(cos((h2.asento)) - sin(h2.asento))),(h2.ypos + oikeasade*(sin(h2.asento) + cos(h2.asento))),50,50)


//ellipse((h1.xpos + oikeakoko/10*h1.suunta*(cos((h2.asento)) - sin(h1.asento))),(h1.ypos + oikeasade*(sin(h1.asento) + cos(h1.asento))),50,50)

//osuukox = ((h2.xpos + oikeakoko/10*h2.suunta*(cos((h2.asento)) - sin(h2.asento))) - (h1.xpos + oikeakoko/10*h1.suunta*(cos((h1.asento)) - sin(h1.asento))))^2;
//osuukoy = ((h2.ypos + oikeasade*(sin(h2.asento) + cos(h2.asento))) - (h1.ypos + oikeasade*(sin(h1.asento) + cos(h1.asento))))^2;

//osuukox = (h1.xpos - h2.xpos)^2;
//osuukoy = (h1.ypos - h2.ypos)^2;



//println(osuukox)

//ellipse(sqrt(osuukox + osuukoy),osuukoy,50,50);

//if (sqrt(osuukox + osuukoy)<oikeakoko) {
//	xv = -xv;
//	yv = -yv;
//        suunta = suunta*-1;
  ...
  void move(key,keyCode) {
	...
	if ((key==52) || (keyCode==38)){
		if (this.energia>0){
      	...
      	if ((key==49) || (keyCode==40)){
	        this.kaasu -=kaasuaskel;
		if (this.kaasu<0) { this.kaasu = 0;}
	   	//println(this.kaasu);
      	}
	if ((key==50)|| (keyCode==37)){
		if ((kaasu > 0) || (ypos>maakorkeus)) {
			asento -=kaantyykerralla*suunta;
	   		s21.rotate(-kaantyykerralla*suunta);  
			s22.rotate(kaantyykerralla*suunta);  	
...
// vaihtoehtonäppikset
if ((keyCode==38) || (keyCode==40)|| (keyCode==37)|| (keyCode==39)|| (keyCode==16)){
   h2.move(key,keyCode);
}
